...
| `server/` | The daemon. Node and TypeScript, binds `127.0.0.1` only, SQLite at `~/.remy/remy.db` through `node:sqlite`. Threads run on the [Claude Agent SDK](https://docs.claude.com/en/api/agent-sdk/overview). |
...
Node 22.5+ for `node:sqlite`, `git`, `gh` authenticated for pull requests, Claude Code on the machine, and `tmux` for the older session remote. Tailscale only if another device needs to reach the daemon.
