...
- **Both providers keep a conversation; their runtimes differ.** A Claude thread currently holds one SDK query process across many turns and can stop mid-turn to ask. Remy's current Codex adapter runs `codex exec --experimental-json` once per turn, then resumes the thread id Codex handed back on the next one; the process ends, but the Codex conversation does not. `codex app-server` is the richer product-integration path when Remy needs a long-lived connection, live steering or interactive requests. Until that adapter is used, Remy holds Codex to a sandbox: Ask stays read-only, Accept edits is `workspace-write`, Bypass is `danger-full-access`. Never quietly grant what a person would have been asked about.
