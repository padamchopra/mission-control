...
export type AgentKind = "shell" | "claude" | "codex" | "cursor";
...
export function agentKind(value: unknown, fallback: AgentKind = "shell"): AgentKind {
  return value === "claude" || value === "codex" || value === "cursor" || value === "shell" ? value : fallback;
}
...
export function agentCommand(agent: AgentKind): string | undefined {
  ...
  for (const directory of new Set(directories)) {
    const candidate = join(directory, agent === "cursor" ? "agent" : agent);
    try {
  ...
  const displayName = agent === "claude" ? "Claude Code" : agent === "cursor" ? "Cursor CLI" : "Codex";
...
export function inferAgent(paneCommand: string, recorded?: AgentKind): AgentKind {
  ...
  if (command.includes("cursor") || /(^|\/)agent(?:\s|$)/.test(command)) return "cursor";
